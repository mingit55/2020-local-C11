class Store {
    ...
    storeUpdate(){
        ...
        if(viewList.length > 0){
            ...
            this.$storeList.html(`<div class="w-100 py-5 text-center text-muted fx-n2">일치하는 상품이 없습니다.</div>`);
        }
    }
    ...
    cartUpdate(){
        if(this.cartList.length > 0){
            ...
            this.$cartList.html(`<div class="w-100 py-5 text-center text-muted fx-n2">장바구니에 담긴 상품이 없습니다.</div>`);
        }

        $(".total-price").html(this.totalPrice.toLocaleString());
    }

    setEvents(){
        let dragTarget, startPoint, timeout;
        this.$storeList.on("dragstart", ".image", function(e){
            e.preventDefault();
